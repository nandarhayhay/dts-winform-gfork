...
#Region " Sub "
    ...
    Private Sub ReadAccess()
        If Not CMain.IsSystemAdministrator Then
            Me.btnAddNew.Enabled = NufarmBussinesRules.User.Privilege.ALLOW_INSERT.SHIP_TO
            If NufarmBussinesRules.User.Privilege.ALLOW_UPDATE.SHIP_TO Then
                Me.grdShipTo.AllowEdit = Janus.Windows.GridEX.InheritableBoolean.True
            Else
                Me.grdShipTo.AllowEdit = Janus.Windows.GridEX.InheritableBoolean.False
            End If
            If NufarmBussinesRules.User.Privilege.ALLOW_DELETE.SHIP_TO Then
                Me.grdShipTo.AllowDelete = Janus.Windows.GridEX.InheritableBoolean.True
            Else
                Me.grdShipTo.AllowDelete = Janus.Windows.GridEX.InheritableBoolean.False
            End If
            Me.btnSave.Enabled = ((NufarmBussinesRules.User.Privilege.ALLOW_INSERT.SHIP_TO = True) Or (NufarmBussinesRules.User.Privilege.ALLOW_UPDATE.SHIP_TO = True) Or (NufarmBussinesRules.User.Privilege.ALLOW_DELETE.SHIP_TO = True))
        End If
    End Sub
                    ...
                    If Not IsNothing(Me.clsShipTo.GetDateSet()) Then
                        If Me.clsShipTo.GetDateSet.HasChanges() Then
                            If MessageBox.Show("Are you sure you want to refresh" & vbCrLf & "All curently any unsaved data will be discarded", "Data has changed !", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = Windows.Forms.DialogResult.No Then
                                Return
                            End If
                            Me.clsShipTo.GetDateSet().RejectChanges()
                        End If
                    End If
                    Me.LoadData()
            End Select
