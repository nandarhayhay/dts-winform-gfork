...
SELECT * FROM NI87.DBO.OEINVH WHERE RTRIM(INVNUMBER) = 'IN17063132'
 -----------===============QUERY Credit Return---==============================================================
 ----POKDI/0415/0013.A
 SELECT CRDH1.INVNUMBER,CRDH1.ITEM,ISNULL(SUM(CRDH1.QTYRETURN),0)AS QTYRETURN
		   FROM 
			(								
                   ...
                   WHERE RTRIM(CRDH1.INVNUMBER) IN('IN17062672') 
                    GROUP BY CRDH1.INVNUMBER,CRDH1.ITEM    
                  --)CRD
				  GO
--===============================================================================================
       ...
       LEFT OUTER JOIN
		   ...
		   FROM 
			(								
                   ...
                   WHERE RTRIM(CRDH1.INVNUMBER) IN('IN17063132') 
                    GROUP BY CRDH1.INVNUMBER,CRDH1.ITEM    
                  )CRD
        ON CRD.INVNUMBER = INVH.INVNUMBER 
        AND CRD.ITEM = INVD.ITEM
        ...
        AND (RTRIM(INVD.ACCTSET) = 'FG' OR RTRIM(INVD.ACCTSET) = 'FGST') AND INVD.LINETYPE = 1
        AND LEN(RIGHT(RTRIM(INVD.ITEM),11)) >= 11
		AND RTRIM(INVH.INVNUMBER) IN('IN17063132') 

		GO
		--=========================================================================================================

		---====================qUERY ke INVOICE Header dengan PO Number---------------------------
		SELECT * FROM NI87.DBO.OEINVH WHERE RTRIM(PONUMBER) = 'SAP/2016/11/017A' 

		---====================qUERY ke INVOICE DETAIL dengan no invoice IN('IN15125113.','IN15125113') 
		SELECT * FROM  NI87.dbo.OEINVD WHERE INVUNIQ  IN(820884)	

		SELECT * FROM  NI87.dbo.OEINVD WHERE INVUNIQ  IN(729734)	

			SELECT * FROM ORDR_PO_BRANDPACK WHERE PO_REF_NO = '17/01/085'
		GO
		---=============================================================================================
		
		--exec Usp_Create_Temp_Invoice_Table @COMPUTERNAME = 'NKusnandar',@DEC_START_DATE = 20161201,@DEC_END_DATE = 20170131
...
--SELECT * FROM NI87.DBO.OEORDH WHERE PONUMBER IN('029/8M/NI/15');   
--SELECT * FROM NI87.DBO.OEORDD WHERE ORDUNIQ IN (1530560);       
--SELECT * FROM NI87.DBO.OEORDD WHERE ORDUNIQ IN (1523649);
    
--SELECT * FROM NI87.DBO.OEORDHO WHERE ORDUNIQ = 1530560
--SELECT CRDH1.INVNUMBER,CRDH1.ITEM,ISNULL(SUM(CRDH1.QTYRETURN),0)AS QTYRETURN
--		   FROM 
--			(								
...
--SELECT * FROM NI87.dbo.ICCATG WHERE [desc] LIKE '%MESO%'

--SELECT * FROM NI87.dbo.ICITEM WHERE [desc] LIKE '%MESO%'
