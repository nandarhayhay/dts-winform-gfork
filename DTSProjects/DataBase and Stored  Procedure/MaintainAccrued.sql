...
--GET PO
--SELECT * FROM NI109.DBO.OEORDH WHERE PONUMBER = '067/II/VN/2015' --'IN15083639.'
--SELECT * FROM NI109.DBO.OEINVH WHERE PONUMBER = '067/II/VN/2015'
----GET REFERENCE
--SELECT * FROM NI109.DBO.OEORDHO WHERE

--SELECT * FROM NI109.DBO.OEINVH WHERE INVDATE >= 20240801 AND INVDATE <= 20241130 INVNUMBER = 'IN23004802.'

--SELECT * FROM NI109.DBO.OEINVD WHERE INVUNIQ in('1684974','1685494')

--SELECT * FROM NI109.DBO.OEINVH WHERE PONUMBER in('006/PKNN/III/2024','007/PKNN/III/2024')
--008/PKNN/III/2024
--009/PKNN/III/2024
--010/PKNN/III/2024
--011/PKNN/III/2024
...
--SELECT * FROM NI109.DBO.OEORDHO WHERE VALUE = '00021613'

--SELECT * FROM NI109.DBO.OEORDH WHERE ORDUNIQ = 1440448

--SELECT * FROM NI109.DBO.ICITEM WHERE [DESC] LIKE '%MICROTHIOL%' AND INACTIVE = 0
...
--SELECT * FROM NI109.DBO.OEINVH WHERE RTRIM(INVNUMBER) = 'IN151225001.'
...
--                            FROM NI109.DBO.OECRDD WHERE LINETYPE = 1 AND (RTRIM(ACCTSET) = 'FG' OR RTRIM(ACCTSET) = 'FGST')
...
-- WHERE AGREEMENT_NO = '118/NI/I/2004.14' AND FLAG = 'S1'
 ...
 --      SELECT INVH.INVDATE,INVH.INVNUMBER,INVH.PONUMBER,RTRIM(ORD1.VALUE)AS REFERENCE,INVD.ITEM,INVD.[DESC] AS BRANDPACK_NAME,INVD.QTYSHIPPED,ISNULL(CRD.QTYRETURN,0)AS QTYRETURN,INVD.UNITPRICE,INVD.AUDTUSER
 --      FROM NI109.dbo.OEINVH INVH INNER JOIN  NI109.dbo.OEINVD INVD ON INVH.INVUNIQ = INVD.INVUNIQ
	--   ( SELECT ITEM,INVUNIQ,ISNULL(SUM(QTYSHIPPED),0) - ISNULL(SUM(QTYBACKORD),0) AS QTYSHIPPED FROM NI109.dbo.OEINVD  WHERE (RTRIM(ACCTSET) = 'FG' OR RTRIM(ACCTSET) = 'FGST') AND LINETYPE = 1 AND LEN(RIGHT(RTRIM(ITEM),11)) >= 11 GROUP BY ITEM,INVUNIQ)INVD1
...
SELECT * FROM NI109.DBO.OEINVH WHERE RTRIM(INVNUMBER) = 'IN23004802'
 -----------===============QUERY Credit Return---==============================================================
 ----POKDI/0415/0013.A
 SELECT CRDH1.INVNUMBER,CRDH1.ITEM,ISNULL(SUM(CRDH1.QTYRETURN),0)AS QTYRETURN
		   FROM 
			(								
                   ...
                   WHERE RTRIM(CRDH1.INVNUMBER) IN('IN23004802') 
                    GROUP BY CRDH1.INVNUMBER,CRDH1.ITEM    
                  --)CRD
				  GO
--===============================================================================================
					    SELECT INVH.INVNUMBER,INVH.PONUMBER,INVD.INVUNIQ,
						INVD.ITEM,INVD.[DESC] AS BRANDPACK_NAME,INVD.QTYSHIPPED,INVD.UNITPRICE
	    FROM NI109.dbo.OEINVD INVD INNER JOIN NI109.DBO.OEINVH INVH ON INVH.INVUNIQ = INVD.INVUNIQ
		WHERE INVH.INVUNIQ IN('1553140','1556530')
...
--------========================QUERY INVOICE by INVNumber==============================================================
       ...
       LEFT OUTER JOIN
		   ...
		   FROM 
			(								
                   ...
                   WHERE RTRIM(CRDH1.INVNUMBER) IN('IN24000132') 
                    GROUP BY CRDH1.INVNUMBER,CRDH1.ITEM    
                  )CRD
        ON CRD.INVNUMBER = INVH.INVNUMBER 
        AND CRD.ITEM = INVD.ITEM
        ...
        AND (RTRIM(INVD.ACCTSET) = 'FG' OR RTRIM(INVD.ACCTSET) = 'FGST') AND INVD.LINETYPE = 1
        AND LEN(RIGHT(RTRIM(INVD.ITEM),11)) >= 11
		AND RTRIM(INVH.INVNUMBER) IN('IN24000132') 

		GO
		--=========================================================================================================
		
		---====================qUERY ke INVOICE Header dengan PO Number---------------------------
		SELECT * FROM NI109.DBO.OEINVH WHERE RTRIM(PONUMBER) = '01/SUT-PB/I/2021' 

		SELECT invuniq,INVNUMBER,INVDATE FROM NI109.DBO.OEINVH WHERE RTRIM(PONUMBER) = '020/BJL-PO/VII/2023' 

			    SELECT INVUNIQ,ITEM,[DESC] AS BRANDPACK_NAME,SUM(QTYSHIPPED) AS QTYSHIPPED,UNITPRICE
	    FROM NI109.dbo.OEINVD WHERE INVUNIQ IN('1553140','1556530')GROUP BY ITEM, INVUNIQ,[DESC],UNITPRICE
		...
		SELECT * FROM  NI109.dbo.OEINVD WHERE INVUNIQ  IN(1084851)	
		
		SELECT * FROM  NI109.dbo.OEINVD WHERE INVUNIQ  IN(1079734)	
  ...
  --                  GROUP BY CRDH1.INVNUMBER,CRDH1.ITEM    
		...
		--		   select * from NI109.DBO.OECRDH WHERE INVUNIQ IN(664558,660292)
		--		   select * from NI109.DBO.OECRDH WHERE INVNUMBER IN('IN15125113.','IN15125113') 

		--00038639
		--19/01/103
		--19/04/006
--SELECT * FROM NI109.DBO.OEORDH WHERE PONUMBER IN('010/NU/DS-SMD/I/2019A');   
--SELECT * FROM NI109.DBO.OEORDD WHERE ORDUNIQ IN (1999617);       
--SELECT * FROM NI109.DBO.OEORDD WHERE ORDUNIQ IN (2029947);
--SELECT * FROM NI109.DBO.OEORDH WHERE ORDUNIQ = '2002304'

--SELECT * FROM NI109.DBO.OEORDHO WHERE ORDUNIQ = 2002304
--SELECT * FROM NI109.DBO.OEORDHO WHERE ORDUNIQ = 2227323
--UPDATE NI109.DBO.OEORDHO SET VALUE = '00044437' WHERE ORDUNIQ = 2227323 AND OPTFIELD = 'DTSREF' ;
...
--SELECT * FROM PLANTATION WHERE PLANTATION_NAME IN(
--SELECT PLANTATION_NAME FROM(
--SELECT PLANTATION_NAME,COUNT(PLANTATION_NAME) AS jumlah
--FROM PLANTATION GROUP BY PLANTATION_NAME
--HAVING COUNT(PLANTATION_NAME) > 1)C)
--ORDER BY PLANTATION_NAME ASC

--SELECT * FROM PLANTATION_GROUP WHERE PLANT_GROUP_ID NOT IN(SELECT PLANT_GROUP_ID FROM PLANTATION WHERE PLANT_GROUP_ID IS NOT NULL)

--SELECT * FROM PLANTATION PL WHERE NOT EXISTS(SELECT PLANTATION_ID FROM ORDR_PO_BRANDPACK WHERE PLANTATION_ID = PL.PLANTATION_ID)
--SELECT * FROM TEMPDB..##T_SELECT_INVOICE_DESKTOPJ9P7A6M
--WHERE BRANDPACK_NAME LIKE 'ROUNDUP 486 SL @ 1 LT-D%'
--DROP TABLE  TEMPDB..##T_SELECT_INVOICE_DESKTOPJ9P7A6M


--exec sp_executesql N'SET DEADLOCK_PRIORITY NORMAL; SET NOCOUNT ON;
--IF NOT EXISTS(SELECT NAME FROM [tempdb].[sys].[objects] WHERE NAME = '##T_SELECT_INVOICE_DESKTOPJ9P7A6M' AND TYPE = 'U') 
--EXEC Usp_Create_Temp_Invoice_Table_NI109
-- @DEC_START_DATE = 20240801,
-- @DEC_END_DATE = 20241130,
-- @COMPUTERNAME = 'DESKTOPJ9P7A6M';
 
-- --END ',N'@UserName varchar(100),@D_START_DATE varchar(8),@D_END_DATE varchar(8),@C_NAME varchar(100)',
-- --@UserName='RSetiorini',@D_START_DATE='20240801',@D_END_DATE='20241130',@C_NAME='DESKTOPJ9P7A6M'


-- SELECT REPLACE(SUBSTRING(REPLACE(RIGHT(RTRIM(INV.ITEM),14),'-',''),2,9),'25HM','025G')AS BRANDPACK_ID,RTRIM(LTRIM(REPLACE(INV.BRANDPACK_NAME,' (HM)',''))) AS BRANDPACK_NAME,
--      QTY = CASE WHEN( INV.QTYSHIPPED - INV.QTYRETURN ) <= 0 THEN 0 ELSE ( INV.QTYSHIPPED - INV.QTYRETURN) END,
--	  RTRIM(INV.INVNUMBER)AS INVNUMBER, RTRIM(INV.PONUMBER) AS PONUMBER,RTRIM(INV.REFERENCE)AS REFERENCE,INV.INVDATE,INV.UNITPRICE, 
--	 (INV.QTYSHIPPED - INV.QTYRETURN)* INV.UNITPRICE AS INV_AMOUNT,INV.AUDTUSER
--      FROM
--      (
--       SELECT INVH.INVDATE,INVH.INVNUMBER,INVH.PONUMBER,RTRIM(ORD1.VALUE)AS REFERENCE,
--	   INVD1.ITEM,INVD1.BRANDPACK_NAME,INVD1.QTYSHIPPED,--ISNULL(CRD.QTYRETURN,0)AS QTYRETURN,
--	   INVD1.UNITPRICE,
--	   INVH.AUDTUSER
...
--        WHERE RTRIM(ORD1.OPTFIELD) = 'DTSREF'
--        AND LEN(RTRIM(ORD1.VALUE)) >= 4
--        AND INVH.INVDATE >= 20240801
--        AND INVH.INVDATE <= 20241130
--	  )INV;


--SELECT GSD.*,BP.BRANDPACK_NAME,QTY_UNIT = CONVERT(VARCHAR(100),QTY) + ISNULL(BPC.UnitOfMeasure,BP.UNIT) FROM GON_SEPARATED_DETAIL GSD  
--                                     INNER JOIN GON_SEPARATED_HEADER GSH ON GSH.IDApp = GSD.FKAppGonHeader LEFT OUTER JOIN(SELECT  BRANDPACK_ID,UnitOfMeasure FROM BRND_PROD_CONV WHERE INACTIVE = 0)BPC ON BPC.BRANDPACK_ID = GSD.ITEM   
--                                     INNER JOIN BRND_BRANDPACK BP ON BP.BRANDPACK_ID = GSD.ITEM WHERE GSH.GON_NUMBER = '002018-02'