...
DECLARE @V_PPN DECIMAL(18,6);
SET @V_PPN = (SELECT TOP 1 PCT_PPN FROM T_PPN WHERE START_DATE <= (SELECT TOP 1 OA_DATE FROM ORDR_ORDER_ACCEPTANCE WHERE OA_ID = @OA_REF_NO) ORDER BY START_DATE DESC)
SET @V_PPN = @V_PPN/100;
...
GO
--=============================================================
--IF EXISTS(SELECT NAME FROM SYS.OBJECTS WHERE NAME = 'Usp_Create_View_Report_GON' AND TYPE = 'P')
--DROP PROCEDURE Usp_Create_View_Report_GON
--GO
--CREATE PROCEDURE Usp_Create_View_Report_GON
--@GON_NO VARCHAR(32),
--@GON_TYPE VARCHAR(50)
--AS
--SET NOCOUNT ON;

IF EXISTS ( SELECT NAME FROM SYS.OBJECTS WHERE NAME = 'Usp_Create_View_Report_AchievementFByPO' AND TYPE = 'P')
DROP PROCEDURE Usp_Create_View_Report_AchievementFByPO
GO
CREATE PROCEDURE Usp_Create_View_Report_AchievementFByPO
@FLAG CHAR(2),
@START_DATE SMALLDATETIME,
@END_DATE SMALLDATETIME,
@ListDistributor BIT,
@ListAgreementNo BIT
AS
SET NOCOUNT ON;SET DEADLOCK_PRIORITY NORMAL; SET NOCOUNT ON; SET ANSI_WARNINGS OFF ;
IF @FLAG = 'F1'
 BEGIN
	IF @ListDistributor = 1 AND @ListAgreementNo = 1
	  BEGIN
		SELECT REG.REGIONAL_AREA,TER.TERRITORY_AREA,DR.DISTRIBUTOR_ID,DR.DISTRIBUTOR_NAME,AA.AGREEMENT_NO,AA.END_DATE,ABI.BRAND_ID,BN.BRAND_NAME,
		ISCombined = CASE WHEN ABI.COMB_AGREE_BRAND_ID IS NOT NULL THEN 'YES' ELSE 'NO' END,
		ABI.TARGET_FMP1,BR_1.BRAND_NAME,ISNULL(BR.ACTUAL,0)AS TOTAL_PO,ISNULL(BR.ACTUAL,0) - ABI.TARGET_FMP1 AS BALANCE FROM AGREE_BRAND_INCLUDE ABI INNER JOIN BRND_BRAND BR_1
		ON ABI.BRAND_ID = BR_1.BRAND_ID
		INNER JOIN AGREE_AGREEMENT AA ON AA.AGREEMENT_NO = ABI.AGREEMENT_NO
		INNER JOIN DISTRIBUTOR_AGREEMENT DA ON DA.AGREEMENT_NO = AA.AGREEMENT_NO
		INNER JOIN DIST_DISTRIBUTOR DR ON DR.DISTRIBUTOR_ID = DA.DISTRIBUTOR_ID
		INNER JOIN TERRITORY TER ON TER.TERRITORY_ID = DR.TERRITORY_ID
        ...
        LEFT OUTER JOIN(
	                    ...
	                    INNER JOIN(
                                    ...
                                    WHERE OPO.PO_REF_DATE <= @END_DATE AND OPO.PO_REF_DATE >= @START_DATE
									AND ABI_2.AGREEMENT_NO IN(SELECT AGREEMENT_NO FROM TEMPDB..##T_Ach_PO_AGREE)	
									AND OPO.DISTRIBUTOR_ID IN(SELECT DISTRIBUTOR_ID FROM TEMPDB..##T_Ach_PO_DIST)
		...
		WHERE ABI.AGREEMENT_NO IN(SELECT AGREEMENT_NO FROM TEMPDB..##T_Ach_PO_AGREE)
		AND DR.DISTRIBUTOR_ID IN(SELECT DISTRIBUTOR_ID FROM TEMPDB..##T_Ach_PO_DIST)
	  END
	  ELSE IF @ListDistributor = 1 AND @ListAgreementNo = 0 --CURRENT AGREEMENT DEFAULT
	  BEGIN
		SELECT REG.REGIONAL_AREA,TER.TERRITORY_AREA,DR.DISTRIBUTOR_ID,DR.DISTRIBUTOR_NAME,AA.AGREEMENT_NO,AA.END_DATE,ABI.BRAND_ID,BN.BRAND_NAME,
		ISCombined = CASE WHEN ABI.COMB_AGREE_BRAND_ID IS NOT NULL THEN 'YES' ELSE 'NO' END,
		ABI.TARGET_FMP1,BR_1.BRAND_NAME,ISNULL(BR.ACTUAL,0)AS TOTAL_PO,ISNULL(BR.ACTUAL,0) - ABI.TARGET_FMP1 AS BALANCE FROM AGREE_BRAND_INCLUDE ABI INNER JOIN BRND_BRAND BR_1
        ...
        LEFT OUTER JOIN(
                        ...
                        ON BR.AGREE_BRAND_ID = ABI.AGREE_BRAND_ID 
		 WHERE DR.DISTRIBUTOR_ID IN(SELECT DISTRIBUTOR_ID FROM TEMPDB..##T_Ach_PO_DIST)
	   END
	   ELSE IF ListDistributor = 0 AND @ListAgreementNo = 1
	   BEGIN
			SELECT REG.REGIONAL_AREA,TER.TERRITORY_AREA,DR.DISTRIBUTOR_ID,DR.DISTRIBUTOR_NAME,AA.AGREEMENT_NO,AA.END_DATE,ABI.BRAND_ID,BN.BRAND_NAME,
			ISCombined = CASE WHEN ABI.COMB_AGREE_BRAND_ID IS NOT NULL THEN 'YES' ELSE 'NO' END,
			ABI.TARGET_FMP1,BR_1.BRAND_NAME,ISNULL(BR.ACTUAL,0)AS TOTAL_PO,ISNULL(BR.ACTUAL,0) - ABI.TARGET_FMP1 AS BALANCE FROM AGREE_BRAND_INCLUDE ABI INNER JOIN BRND_BRAND BR_1
