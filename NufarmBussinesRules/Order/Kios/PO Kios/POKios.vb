            ...
            Try
                ...
                If (Not StartYear <= 0) Then
                    Query = "SET NOCOUNT ON; " & vbCrLf & _
                            "SELECT [NAME] FROM tempdb..Sysobjects WHERE [NAME] = '" & " ##POKios_" & StartYear.ToString() & "_" & EndYear.ToString() & "' "
                    Me.AddParameter("@stmt", SqlDbType.NVarChar, Query)
                    ...
                    Query = "SET NOCOUNT ON;" & vbCrLf & _
                            "SELECT [ROWS] FROM tempdb.dbo.Sysindexes  WHERE id = OBJECT_ID('" & "TEMPDB..##POKios_" & StartYear.ToString & "_" & EndYear.ToString() & "') AND INDID < 2 ;"
            ...
            Try
                If Me.ListYear.Count > 0 Then
                    ...
                    For i As Integer = 0 To ListYear.Count - 1
                        ...
                        Query = "SET NOCOUNT ON;" & vbCrLf & _
                                " SELECT [NAME] FROM tempdb.dbo.sysobjects WHERE [NAME] LIKE '##POKIOS_" & ListYear(i) & "%';"
                        Me.AddParameter("@stmt", SqlDbType.NVarChar, Query)
