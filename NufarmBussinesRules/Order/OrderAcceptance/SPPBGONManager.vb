        ...
        Public Function getDailyGonReport(ByVal Year As Integer, ByVal Month As Integer, Optional ByVal theDate As Object = Nothing) As System.Data.DataTable
            ...
            Try
                Query = "SET NOCOUNT ON;" & vbCrLf & _
                        ...
                        " CONVERT(DECIMAL(16,2),(ISNULL(GD.GON_QTY,0)/ISNULL(SB.SPPB_QTY,0)) * OPB.PO_ORIGINAL_QTY) AS SALES_QTY," & vbCrLf & _
                        " GON_DISC_QTY = CONVERT(DECIMAL(16,2),ISNULL(GD.GON_QTY,0) - ((ISNULL(GD.GON_QTY,0)/ISNULL(SB.SPPB_QTY,0)) * OPB.PO_ORIGINAL_QTY)), " & vbCrLf & _
        ...
        ByVal SPPBNO As Object, ByVal PONUmber As Object, ByVal GONNUmber As Object, ByVal FromDate As Object, ByVal UntilDate As Object, ByVal MustReloadData As Boolean) As DataSet
            ...
            Try
                ...
                Query = "SET NOCOUNT ON; SET ARITHABORT OFF; SET ANSI_WARNINGS OFF ; " & vbCrLf & _
                        ...
                        " CONVERT(DECIMAL(16,2),(ISNULL(GD.GON_QTY,0)/ISNULL(TFR.SPPB_QTY,0)) * TFR.PO_ORIGINAL_QTY) AS SALES_QTY," & vbCrLf & _
                        " GON_DISC_QTY =CONVERT(DECIMAL(16,2), ISNULL(GD.GON_QTY,0) - ((ISNULL(GD.GON_QTY,0)/ISNULL(TFR.SPPB_QTY,0)) * TFR.PO_ORIGINAL_QTY)),GD.BatchNo,GD.UnitOfMeasure,GD.UNIT1,GD.VOL1,GD.UNIT2,GD.VOL2,GD.IsOpen,GH.WARHOUSE,GH.POLICE_NO_TRANS,GH.DRIVER_TRANS," & vbCrLf & _
        ...
        Public Function getDTSPPBGON(ByVal MasterCategory As String, ByVal CustomCategory As String, ByVal PONumber As String, ByVal SPPBNO As Object, ByVal GONNUmber As Object, ByVal FromDate As Object, ByVal UntilDate As Object, ByVal MustReloadData As Boolean) As DataTable
            ...
            Try
                ...
                Query = "SET NOCOUNT ON; SET ARITHABORT OFF; SET ANSI_WARNINGS OFF ; " & vbCrLf & _
                        ...
                        " CONVERT(DECIMAL(16,2),(ISNULL(GD.GON_QTY,0)/ISNULL(TFR.SPPB_QTY,0)) * TFR.PO_ORIGINAL_QTY) AS SALES_QTY," & vbCrLf & _
                        " GON_DISC_QTY = CONVERT(DECIMAL(16,2),ISNULL(GD.GON_QTY,0) - ((ISNULL(GD.GON_QTY,0)/ISNULL(TFR.SPPB_QTY,0)) * TFR.PO_ORIGINAL_QTY)),GD.BatchNo,GH.WARHOUSE,GH.POLICE_NO_TRANS,GH.DRIVER_TRANS," & vbCrLf & _
            ...
            Try
                ...
                If Not IsForEntrySPPB Then
                    ...
                    Query = "SET NOCOUNT ON; SET ARITHABORT OFF; SET ANSI_WARNINGS OFF ;  " & vbCrLf & _
                             ...
                             "  SALES_QTY = CASE WHEN GH.GON_QTY IS NULL THEN TFR.PO_ORIGINAL_QTY ELSE CONVERT(DECIMAL(16,2),(ISNULL(GH.GON_QTY,0)/ISNULL(TFR.SPPB_QTY,0)) * TFR.PO_ORIGINAL_QTY) END, " & vbCrLf & _
                             " GON_DISC_QTY = CONVERT(DECIMAL(16,2),ISNULL(GH.GON_QTY,0) - ((ISNULL(GH.GON_QTY,0)/ISNULL(TFR.SPPB_QTY,0)) * TFR.PO_ORIGINAL_QTY)), GH.BatchNo,GH.UnitOfMeasure,GH.UNIT1,GH.VOL1,GH.UNIT2,GH.VOL2,GH.IsOpen,GH.WARHOUSE,GH.POLICE_NO_TRANS,GH.DRIVER_TRANS," & vbCrLf & _
                             " IsCompleted = CONVERT(BIT,(CASE WHEN (GRB.GRB_ID IS NOT NULL) THEN 1 ELSE 0 END))," & vbCrLf & _
                ...
                Else
                    Query = "SET NOCOUNT ON; SET ARITHABORT OFF; SET ANSI_WARNINGS OFF ; " & vbCrLf & _
                            ...
                            " SALES_QTY = CASE WHEN (GD.GON_QTY IS NULL) THEN TFR.PO_ORIGINAL_QTY ELSE CONVERT(DECIMAL(16,2),(ISNULL(GD.GON_QTY,0)/ISNULL(TFR.SPPB_QTY,0)) * TFR.PO_ORIGINAL_QTY) END, " & vbCrLf & _
                            " GON_DISC_QTY = CONVERT(DECIMAL(16,2),ISNULL(GD.GON_QTY,0) - ((ISNULL(GD.GON_QTY,0)/ISNULL(TFR.SPPB_QTY,0)) * TFR.PO_ORIGINAL_QTY)),GD.BatchNo,GD.UnitOfMeasure,GD.UNIT1,GD.VOL1,GD.UNIT2,GD.VOL2,GD.IsOpen,GH.WARHOUSE,GH.POLICE_NO_TRANS,GH.DRIVER_TRANS," & vbCrLf & _
                            " IsCompleted = CONVERT(BIT,(CASE WHEN (GRB.GRB_ID IS NOT NULL) THEN 1 ELSE 0 END))," & vbCrLf & _
                            " GRB.RETURNED_GON_DATE,GAP = CASE WHEN (GRB.GRB_ID IS NOT NULL) THEN (DATEDIFF(DAY,GH.GON_DATE,GRB.RETURNED_GON_DATE)) ELSE NULL END, GD.CreatedBy,GD.CreatedDate,GD.IsUpdatedBySystem " & vbCrLf & _
            ...
            Try
                If Not IsForEntrySPPB Then
                    ...
                    Query = "SET NOCOUNT ON; SET ARITHABORT OFF; SET ANSI_WARNINGS OFF ;  " & vbCrLf & _
                             ...
                             " GH.GON_NO,GH.GON_DATE, E_T_A = CASE WHEN GA.DAYS_RECEIPT IS NULL THEN NULL ELSE DATEADD(DAY,(1 + GA.DAYS_RECEIPT),GH.GON_DATE) END,GT.GT_ID,GA.GON_ID_AREA, GA.AREA,GT.TRANSPORTER_NAME,TFR.SPPB_BRANDPACK_ID,GH.GON_DETAIL_ID,GH.GON_QTY, " & vbCrLf & _
                             " SALES_QTY = CASE WHEN GH.GON_QTY IS NULL THEN TFR.PO_ORIGINAL_QTY ELSE CONVERT(DECIMAL(16,2),(ISNULL(GH.GON_QTY,0)/ISNULL(TFR.SPPB_QTY,0)) * TFR.PO_ORIGINAL_QTY) END," & vbCrLf & _
                             " GON_DISC_QTY = CONVERT(DECIMAL(16,2),ISNULL(GH.GON_QTY,0) - ((ISNULL(GH.GON_QTY,0)/ISNULL(TFR.SPPB_QTY,0)) * TFR.PO_ORIGINAL_QTY)), " & vbCrLf & _
                             " GH.BatchNo,GH.UnitOfMeasure,GH.UNIT1,GH.VOL1,GH.UNIT2,GH.VOL2,GH.IsOpen,GH.WARHOUSE,GH.POLICE_NO_TRANS,GH.DRIVER_TRANS," & vbCrLf & _
                ...
                Else
                    Query = "SET NOCOUNT ON; SET ARITHABORT OFF; SET ANSI_WARNINGS OFF ; " & vbCrLf & _
                            ...
                            " SALES_QTY = CASE WHEN (GD.GON_QTY IS NULL) THEN TFR.PO_ORIGINAL_QTY ELSE CONVERT(DECIMAL(16,2),(ISNULL(GD.GON_QTY,0)/ISNULL(TFR.SPPB_QTY,0)) * TFR.PO_ORIGINAL_QTY) END," & vbCrLf & _
                            " GON_DISC_QTY = CONVERT(DECIMAL(16,2),ISNULL(GD.GON_QTY,0) - ((ISNULL(GD.GON_QTY,0)/ISNULL(TFR.SPPB_QTY,0)) * TFR.PO_ORIGINAL_QTY)),GD.BatchNo,GD.UnitOfMeasure,GD.UNIT1,GD.VOL1,GD.UNIT2,GD.VOL2,GD.IsOpen," & vbCrLf & _
                            " TOTAL_SALES_VALUE = (CASE WHEN (GD.GON_QTY IS NULL) THEN TFR.PO_ORIGINAL_QTY ELSE CONVERT(DECIMAL(16,2),(ISNULL(GD.GON_QTY,0)/ISNULL(TFR.SPPB_QTY,0)) * TFR.PO_ORIGINAL_QTY) END)* TFR.PRICE, IsCompleted = CONVERT(BIT,(CASE WHEN (GRB.GRB_ID IS NOT NULL) THEN 1 ELSE 0 END))," & vbCrLf & _
                            " GRB.RETURNED_GON_DATE,GAP = CASE WHEN (GRB.GRB_ID IS NOT NULL) THEN (DATEDIFF(DAY,GH.GON_DATE,GRB.RETURNED_GON_DATE)) ELSE NULL END, GD.CreatedBy,GD.CreatedDate,GD.IsUpdatedBySystem " & vbCrLf & _
                            " FROM TEMPDB..##T_F_R_SPPB_" & Me.ComputerName & " TFR INNER JOIN GON_HEADER GH ON GH.SPPB_NO = TFR.SPPB_NO INNER JOIN GON_DETAIL GD ON GH.GON_HEADER_ID = GD.GON_HEADER_ID AND GD.SPPB_BRANDPACK_ID = TFR.SPPB_BRANDPACK_ID " & vbCrLf & _
                            " LEFT OUTER JOIN GON_AREA GA ON GA.GON_ID_AREA = GH.GON_ID_AREA " & vbCrLf & _
                            " LEFT OUTER JOIN GON_TRANSPORTER GT ON GT.GT_ID = GH.GT_ID " & vbCrLf & _
