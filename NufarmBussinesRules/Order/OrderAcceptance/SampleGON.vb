...
Namespace OrderAcceptance
    Public Class SampleGON
        ...
        End Sub
        Public Function getTotalGon(ByVal PORefNo As String, ByVal IDAppItemOther As Integer, ByVal IDpp As Object, ByVal closeConnection As Boolean) As Decimal
            Try
            ...
            End Try
        End Function
        Public Function getTotalGon(ByVal PORefNo As String, ByVal ItemOther As Integer, ByVal closeConnection As Boolean) As Decimal
            Try
                Query = "SET NOCOUNT ON;" & vbCrLf & _
                " SELECT ISNULL(SUM(GSD.QTY),0) AS TOTAL FROM GON_SEPARATED_DETAIL GSD INNER JOIN GON_SEPARATED_PO_DETAIL GSPD ON GSD.FKAppPODetail = GSPD.IDApp " & vbCrLf & _
        ...
        Public Function getPODetail(ByVal PoRefNO As String, ByVal CloseConnection As Boolean) As DataView
            Try
                ...
                " INNER JOIN GON_SEPARATED_PO_HEADER GSPH ON GSPH.IDApp = GSPD.FKApp " & vbCrLf & _
                " LEFT OUTER JOIN(SELECT FKAppPODetail,SUM(QTY) AS QTY FROM GON_SEPARATED_DETAIL WHERE ITEM_OTHER IS NOT NULL GROUP BY FKAppPODetail)IGSD ON IGSD.FKAppPODetail = GSPD.IDApp WHERE GSPH.PO_NUMBER = @PO_NUMBER ;"
                If IsNothing(Me.SqlCom) Then : Me.CreateCommandSql("", Query)
                Else : Me.ResetCommandText(CommandType.Text, Query)
                End If
                Me.AddParameter("@PO_NUMBER", SqlDbType.VarChar, PoRefNO)
        ...
        ByRef DtPOdetail As DataTable, ByRef dtGonDetail As DataTable) As Boolean
            ...
            Try
                ...
                If Mode = SaveMode.Update And HasChangedPOdetail Then
                    ...
                    " [PO_NUMBER] [varchar] (50),[ITEM_OTHER] [INT],[IDApp][INT] )" '& vbCrLf & _
                ...
                If Mode = SaveMode.Insert Then
                    ...
                    With commandInsert
                        ...
                        End If
                        Dim listItemOther As New List(Of String)
                        If Not IsNothing(DtPOdetail) Then
                            ...
                            If POInsertedRows.Length > 0 Then
                                For Each rows As DataRow In POInsertedRows
                                    ...
                                    .Parameters.Add("@FKApp", SqlDbType.Int, 0).Value = IDAppSPPBHeader
                                    Dim IDAppItemOther As Integer = rows("ITEM_OTHER")
                                    Dim status As String = rows("STATUS")
                                    .Parameters.Add("@ITEM_OTHER", SqlDbType.Int).Value = IDAppItemOther
                                    ...
                                    If Not IsNothing(dtGonDetail) And HasChangedGONDetail Then
                                        Dim foundRows() As DataRow = dtGonDetail.Select("ITEM_OTHER = " & IDAppItemOther)
                                        If foundRows.Length > 0 Then
                                            ...
                                            .Parameters.Add("@ITEM_OTHER", SqlDbType.Int).Value = foundRows(0)("ITEM_OTHER")
                                            ...
                                            .ExecuteScalar()
                                            If Not listItemOther.Contains(IDAppItemOther) Then
                                                listItemOther.Add(IDAppItemOther)
                                            End If
                                            .Parameters.Clear()
                                            SaveGonDetail = True
                                        End If
                            ...
                            If UpdatedRows.Length > 0 Then
                                With commandUpdate
                                    ...
                                    .Parameters.Add("@ITEM_OTHER", SqlDbType.Int, 0, "ITEM_OTHER")
                            ...
                            End If
                        End If
                        Dim IDAppItemOtherS As String = "ITEM NOT IN("
                        If listItemOther.Count > 0 Then
                            For i As Integer = 0 To listItemOther.Count - 1
                                IDAppItemOtherS &= listItemOther(i)
                                If i < listItemOther.Count - 1 Then
                                    IDAppItemOtherS &= ","
                                End If
                            Next
                            IDAppItemOtherS &= ")"
                        ...
                        If HasChangedGONDetail Then
                            InsertedRows = dtGonDetail.Select(IIf(listItemOther.Count > 0, IDAppItemOtherS, ""), "", DataViewRowState.Added)
