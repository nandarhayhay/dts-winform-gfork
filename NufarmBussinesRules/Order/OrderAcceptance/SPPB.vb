        ...
        Public Function getPO(ByVal SearchPO As String, ByVal Mode As SaveMode, ByVal MustCloseConnection As Boolean) As DataTable
            Try
                ...
                If Mode = SaveMode.Insert Then
                    Query = "SET NOCOUNT ON; " & vbCrLf & _
                               " SELECT TOP 100 PO.PO_REF_NO,PO.PO_REF_DATE AS PO_DATE,DR.DISTRIBUTOR_NAME,ISNULL(DR.ADDRESS,'UNKNOW ADDRESS')AS ADDRESS FROM ORDR_PURCHASE_ORDER PO INNER JOIN ORDR_ORDER_ACCEPTANCE OA " & vbCrLf & _
                               ...
                               " AND PO.PO_REF_NO <> ALL(SELECT PO_REF_NO FROM SPPB_HEADER) AND DATEDIFF(MONTH,PO.PO_REF_DATE,GETDATE()) <= 8 AND EXISTS(SELECT OA_ID FROM ORDR_OA_BRANDPACK WHERE OA_ID = OA.OA_ID AND OA_ORIGINAL_QTY > 0);"
                ElseIf Mode = SaveMode.Update Then
                    Query = "SET NOCOUNT ON; " & vbCrLf & _
                            " SELECT TOP 1 PO.PO_REF_NO,PO.PO_REF_DATE AS PO_DATE,DR.DISTRIBUTOR_NAME,ISNULL(DR.ADDRESS,'UNKNOW ADDRESS')AS ADDRESS FROM ORDR_PURCHASE_ORDER PO INNER JOIN ORDR_ORDER_ACCEPTANCE OA " & vbCrLf & _
